...
if (config.lark.mock) {
  ...
  mark(line(allowMock, `飞书：MOCK 模式${allowMock ? "（本次允许）" : "，需要显式传 --allow-mock"}`));
} else {
  mark(line(Boolean(config.lark.appId && config.lark.appSecret), "LARK_APP_ID / LARK_APP_SECRET"));
  line(true, config.lark.userOpenId ? "LARK_USER_OPEN_ID（准备文档授权用）" : "LARK_USER_OPEN_ID 未配置，将跳过准备文档授权");
}
